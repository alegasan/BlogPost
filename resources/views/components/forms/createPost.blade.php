...
<section class="rounded-3xl border border-[#d7d1c6] bg-[#fffdf7] p-4 sm:p-6">
    ...
    <form class="mt-6 grid gap-4 md:grid-cols-2" 
		...
		x-data="{ submitting: null, status: 'published' }"
        ...
        <div class="flex flex-col items-stretch justify-end gap-2 sm:flex-row sm:items-center md:col-span-2">
            ...
            <x-ui.button variant="secondary" size="sm" type="submit" class="w-full sm:w-auto"
                x-bind:disabled="submitting !== null" 
				x-on:click="status = 'draft'">
                <span x-show="submitting !== 'draft'">Save Draft</span>
                <span x-show="submitting === 'draft'">Saving...</span>
            </x-ui.button>


            <x-ui.button size="sm" type="submit" class="w-full sm:w-auto" 
				x-bind:disabled="submitting !== null"
                x-on:click="status = 'published'">
                <span x-show="submitting !== 'published'">Publish</span>
                <span x-show="submitting === 'published'">Publishing...</span>
            </x-ui.button>
        </div>
    </form>
</section>
